...
def create_app() -> Flask:
    ...
    def scrum_api_item_detail():
        ...
        _kind_map = {r[0]: {"label": r[1], "color": r[2]} for r in SCRUM_BUILTIN_TASK_KIND_ROWS}
        conn = _open_db()
        try:
            row = conn.execute(
                """
                SELECT i.id, i.title, i.estimate_hours, i.assignee,
                       i.task_kind, i.area, i.kanban_column,
                       COALESCE(SUM(a.committed_hours), 0) AS burnt_hours
                FROM scrum_sprint_item i
                LEFT JOIN scrum_item_activity a ON a.item_id = i.id
                WHERE i.id = ?
                GROUP BY i.id
                """,
                (item_id,),
            ...
            links = [
                {"label": lf["label"] or lf["url"], "url": lf["url"]}
                for lf in conn.execute(
                    "SELECT label, url FROM scrum_item_linked_file WHERE item_id = ? ORDER BY id",
                    (item_id,),
                ).fetchall()
            ]
            activity = [
                {
                    "body": ac["body"] or "",
                    "committed_hours": float(ac["committed_hours"] or 0),
                    "from_column": ac["from_column"] or "",
                    "to_column": ac["to_column"] or "",
                    "created_at": ac["created_at"] or "",
                }
                for ac in conn.execute(
                    """SELECT body, committed_hours, from_column, to_column, created_at
                       FROM scrum_item_activity WHERE item_id = ?
            ...
            conn.close()
        kind_info = _kind_map.get(row["task_kind"], {"label": row["task_kind"].upper(), "color": "#64748b"})
        est = float(row["estimate_hours"] or 0)
        burnt = float(row["burnt_hours"] or 0)
        burn_pct = round((burnt / est) * 100) if est > 0 else 0
        return jsonify({
            "ok": True,
            "item": {
                "id": row["id"],
                ...
                "kind_label": kind_info["label"],
                "kind_color": kind_info["color"],
                "area": row["area"] or "",
